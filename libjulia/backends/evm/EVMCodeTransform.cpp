...
{
	solAssert(m_evm15, "Function calls not supported for EVM 1.0");
	solAssert(m_scope, "");
	...
	for (auto it = _call.arguments.rbegin(); it != _call.arguments.rend(); ++it)
		visitExpression(*it);
...
void CodeTransform::operator()(FunctionalInstruction const& _instruction)
{
	...
	{
		for (auto it = _instruction.arguments.rbegin(); it != _instruction.arguments.rend(); ++it)
			visitExpression(*it);
...
void CodeTransform::operator()(FunctionDefinition const& _function)
{
	...
	m_assembly.appendJumpTo(afterFunction);
	...
	solAssert(varScope, "");
	//@TODO I think arguments should be reversed
	for (auto const& v: _function.arguments + _function.returns)
	{
	...
	for (size_t i = 0; i < function.returns.size(); ++i)
		m_assembly.appendConstant(u256(0));
	CodeTransform(m_errors, m_assembly, m_info, true, m_identifierAccess, height).run(_function.body);

	if (function.arguments.size() > 0)
	{
		vector<int> stackLayout;
		stackLayout += vector<int>(function.arguments.size(), -1); // discard all arguments
		for (size_t i = 0; i < function.returns.size(); ++i)
